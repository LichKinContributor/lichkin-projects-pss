...
@Service("SysPssSellReturnNotStockOutOrderProductL00Service")
public class S extends LKApiBusGetListService<I, O, SysPssSellReturnNotStockOutOrderProductEntity> {

	@Override
	protected void initSQL(I sin, ApiKeyValues<I> params, QuerySQL sql) {
		// 主表
		// sql.select(SysPssSellReturnNotStockOutOrderProductR.id);
		sql.select(SysPssSellReturnNotStockOutOrderProductR.quantity);

		// 关联表
		sql.innerJoin(SysPssSellOrderProductEntity.class, new Condition(SysPssSellReturnNotStockOutOrderProductR.sellOrderProductId, SysPssSellOrderProductR.id));
		...
		sql.innerJoin(SysPssProductEntity.class, new Condition(SysPssProductR.id, SysPssSellReturnNotStockOutOrderProductR.productId));
		...
		// 筛选条件（必填项）
		// addConditionId(sql, SysPssSellReturnNotStockOutOrderProductR.id, params.getId());
		// addConditionLocale(sql, SysPssSellReturnNotStockOutOrderProductR.locale, params.getLocale());
		// addConditionCompId(true, sql, SysPssSellReturnNotStockOutOrderProductR.compId, params.getCompId(), params.getBusCompId());
		// addConditionUsingStatus(true,params.getCompId(), sql, SysPssSellReturnNotStockOutOrderProductR.usingStatus, params.getUsingStatus(), LKUsingStatusEnum.STAND_BY, LKUsingStatusEnum.USING);

		// 筛选条件（业务项）
		String orderId = sin.getOrderId();
		if (StringUtils.isNotBlank(orderId)) {
			sql.eq(SysPssSellReturnNotStockOutOrderProductR.orderId, orderId);
		}

		// 排序条件
		sql.addOrders(new Order(SysPssSellReturnNotStockOutOrderProductR.id, false));
	}
